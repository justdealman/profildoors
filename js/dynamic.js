...
function introtext() {
	$('.intro .text').width($('.wrapper').width());
}
function articletext() {
	$('.articles .text').width($('.wrapper').width());
}
function shadow() {
	$('.lshadow, .rshadow').width(($(window).width()-1360)/2);
}
...
$(document).ready(function() {
	...
	if ( $('.index').length > 0 ) {
		...
		$('.logo').parent().detach().prependTo('.index > div');
	}
	if ( $('.catalog').length > 0 ) {
		catalog();
		...
		$('.logo').parent().detach().prependTo('.catalog');
	}
	if ( $('.list').length > 0 ) {
		list();
	}
	...
	if ( $('.intro').length > 0 ) {
		$('.logo').parent().detach().prependTo('.intro');
	}
	if ( $('.articles').length > 0 ) {
		articles();
		$('.logo').parent().detach().prependTo('.articles');
	}
	if ( $('.intro .text').length > 0 ) {
		introtext();
	}
	...
	$('body').append('<div class="lshadow"></div>');
	$('body').append('<div class="rshadow"></div>');
	shadow();
});